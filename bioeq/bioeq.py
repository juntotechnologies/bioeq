...
class BioEq:
    ...
    def _validate_colvals(self):
        pass  # fill in later, for now assume everything is good
    ...
    def compute_auc(self):
        ...
        auc_df = df.group_by(self.subject_col).agg(
            pl.struct([self.time_col, self.conc_col])
            .apply(
    ...
    def compute_ratio(
        ...
        joined_df = df.join(auc_df, on="Subject", how="inner").unique(
            subset=["Subject", group_col]
        )
        # Pivot wide to compare test_label vs ref_label
        pivoted = joined_df.pivot(
            values=measure_col, index="Subject", columns=group_col
        )
    ...
    def run_tost(
        ...
        results = {
            ...
            "within_80_125": (lower_bound >= lower_limit)
            and (upper_bound <= upper_limit),
        }
        return results
