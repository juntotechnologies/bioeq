...
@pytest.fixture
def simulated_crossover_data():
    """Fixture to provide simulated crossover design data for testing"""
    try:
        # First try to load from file
        file_path = Path("simdata/crossover_simdata.csv")
        if file_path.exists():
    ...
    try:
        from simdata.simulation_data_generator import generate_crossover_data
        return generate_crossover_data(n_subjects=12)
    except ImportError:
        # Create minimal test data if the generator is not available
        data = []
        for subject in range(1, 5):  # 4 subjects
            sequence = "TR" if subject % 2 == 0 else "RT"
...
def test_crossover2x2_initialization(simulated_crossover_data):
    """Test that the Crossover2x2 class initializes correctly"""
    analyzer = Crossover2x2(
        data=simulated_crossover_data,
    ...
    expected_rows = len(simulated_crossover_data["SubjectID"].unique()) * 2  # subjects * periods
    assert len(analyzer.params_df) == expected_rows, f"Expected {expected_rows} rows, got {len(analyzer.params_df)}"


def test_pk_parameter_calculation(simulated_crossover_data):
    ...
    subject = simulated_crossover_data["SubjectID"].unique()[0]
    period = 1
    subject_data = simulated_crossover_data.filter(
...
def test_run_anova(simulated_crossover_data):
    ...
    assert "Period" in result["formula"]
    assert "Sequence" in result["formula"]


def test_run_nlme(simulated_crossover_data):
    """Test that the mixed effects model function runs without errors"""
    analyzer = Crossover2x2(
        data=simulated_crossover_data,
        subject_col="SubjectID",
        seq_col="Sequence",
    ...
    # Run mixed effects model on log-transformed AUC
    result = analyzer.run_nlme("log_AUC")
    
    # Check that result is a dictionary
    assert isinstance(result, dict)
    
    # Check that model_summary exists in the result
...
def test_point_estimate(simulated_crossover_data):
...
def test_summarize_pk_parameters(simulated_crossover_data):
